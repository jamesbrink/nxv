...
- `publisher.rs`: Generates compressed index files (slim + full variants), bloom filter, and signed manifest for distribution
...
Schema version is `SCHEMA_VERSION = 3`. Database uses WAL mode with 5-second busy timeout.
...
- `index.db` - SQLite database with package versions (slim variant by default,
  or full if `index:full` was used)
- `bloom.bin` - Bloom filter for fast negative lookups

Index variants (generated by `nxv publish`):

- **Slim** (`index.db.zst`): One row per (attribute_path, version) - ~28MB compressed, 31x smaller
- **Full** (`index-full.db.zst`): All version ranges preserved - ~1.3GB compressed

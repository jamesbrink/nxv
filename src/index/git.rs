...
impl NixpkgsRepo {
    ...
    /// Reset the repository to a clean state by hard-resetting to `target`
    /// and removing all untracked files and directories.
    ///
    /// Callers should use [`fetch_nixpkgs_unstable_commit`] to get the current
    /// nixpkgs-unstable channel commit as the target. Falls back to `origin/master`
    /// if no target is provided, but this is not recommended.
    ///
    /// # Errors
    ...
    ///
    /// ```
    /// // Reset to nixpkgs-unstable commit (recommended)
    /// let commit = fetch_nixpkgs_unstable_commit().unwrap();
    /// repo.reset_hard(Some(&commit)).unwrap();
    ///
    ...
    pub fn reset_hard(&self, target: Option<&str>) -> Result<()> {
        ...
        // Default to origin/master as fallback (callers should provide nixpkgs-unstable commit)

...
pub struct IndexerConfig {
    ...
    /// Whether to recursively extract nested package scopes.
...
impl Indexer {
    ...
    fn process_commits_parallel<P: AsRef<Path>>(
        ...
        // Clean up any orphaned worktree sessions from previous runs
        WorktreeSession::cleanup_orphaned_sessions();

        // Create worktree session
        let session = WorktreeSession::new(nixpkgs_path)?;
            ...
            handles.push(std::thread::spawn(move || {
                ...
                while let Ok(task) = rx.recv() {
                    ...
                    let result = std::panic::catch_unwind(AssertUnwindSafe(|| {
                        ...
                        // Build file→attr map for this commit
                        let file_attr_map = build_file_attr_map(&worktree_path, &worker_systems)?;

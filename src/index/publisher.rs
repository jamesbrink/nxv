...
///
/// Creates:
/// - `index.db.zst` - Compressed database (one row per attr+version)
...
/// Generate a compressed index for distribution.
///
/// This compresses the database and returns the index file metadata along with
/// the last indexed commit hash.
fn generate_index<P: AsRef<Path>, Q: AsRef<Path>>(
    db_path: P,
    output_dir: Q,
    url_prefix: Option<&str>,
    show_progress: bool,
...
mod tests {
    ...
    fn test_generate_index() {
        ...
        let (index_file, last_commit) =
            generate_index(&db_path, &output_dir, None, false, None, 19).unwrap();
    ...
    #[test]
    fn test_generate_index_with_url_prefix() {
        let dir = tempdir().unwrap();
        let db_path = dir.path().join("test.db");
        let output_dir = dir.path().join("output");
        ...
        let (index_file, _) =
            generate_index(&db_path, &output_dir, Some(url_prefix), false, None, 19).unwrap();
    ...
    fn test_publish_index_without_signing() {
        ...
        publish_index(&db_path, &output_dir, None, false, None, None, 1).unwrap();

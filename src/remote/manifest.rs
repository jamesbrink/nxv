...
pub struct Manifest {
    ...
    /// Set this to allow older clients to read indexes with additive schema changes
    /// (e.g., new columns that don't break backward compatibility).
    ///
    ...
    /// The index file (database with one row per attr+version).
